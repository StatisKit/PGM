...
{   
    namespace pgm
    {
        ...
        template<class S>
            ...
            {
                bool clique = true;
                typename S::const_iterator itu = u.cbegin(), itu_end = u.cend();
                while(clique && itu != itu_end)
                {
                    typename S::const_iterator itv = u.cbegin();
                    while(clique && itv != itu)
                    {
                        clique = has_edge(*itu, *itv);
                        ++itv;
                    }
                    ++itu;
                }
